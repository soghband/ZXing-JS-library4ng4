...
var DecodedBitStreamParser_1 = require("./../../../../core/qrcode/decoder/DecodedBitStreamParser");
var BitSourceBuilder_1 = require("./../../common/BitSourceBuilder");
...
describe('DecodedBitStreamParser', function () {
    it('testSimpleByteMode', function () {
        var builder = new BitSourceBuilder_1.default();
        builder.write(0x04, 4); // Byte mode
        builder.write(0x03, 8); // 3 bytes
        builder.write(0xF1, 8);
        builder.write(0xF2, 8);
        builder.write(0xF3, 8);
        var result = DecodedBitStreamParser_1.default.decode(builder.toByteArray(), Version_1.default.getVersionForNumber(1), null, null).getText();
    ...
    describe('testECIISOEach', function () {
        var _loop_1 = function (d) {
            it('testECIISOEach ' + d[0], function () {
                testEciOneEncoding(d[0], d[1], d[2], d[3], d[4], d[5]);
            });
        };
        for (var _i = 0, eciTestData_1 = eciTestData; _i < eciTestData_1.length; _i++) {
            var d = eciTestData_1[_i];
            _loop_1(d);
        }
    });
    function testEciOneEncoding(encodingLabel, eciBits, b1, b2, b3, expected) {
        var builder = new BitSourceBuilder_1.default();
        builder.write(0x07, 4); // ECI mode
        ...
        builder.write(b2, 8);
        builder.write(b3, 8);
        var byteArray = builder.toByteArray();
        var result = DecodedBitStreamParser_1.default.decode(byteArray, Version_1.default.getVersionForNumber(1), null, null).getText();
        assert.strictEqual(result, expected, encodingLabel);
    }
    describe('testECIISOCombine', function () {
        var r = new Random_1.default('ECIISO');
        var _loop_2 = function (i) {
            var id1 = r.next(eciTestData.length);
            var id2 = r.next(eciTestData.length);
            var d1 = eciTestData[id1];
            var d2 = eciTestData[id2];
            it('testECIISOCombine ' + d1[0] + ' & ' + d2[0], function () {
                testEciComboned(d1[0], d1[1], d1[2], d1[3], d1[4], d1[5], d2[0], d2[1], d2[2], d2[3], d2[4], d2[5]);
            });
        };
        for (var i = 0; i !== 10; i++) {
            _loop_2(i);
    ...
    function testEciComboned(encodingLabel1, eciBits1, b1, b2, b3, expected1, encodingLabel2, eciBits2, b4, b5, b6, expected2) {
        var builder = new BitSourceBuilder_1.default();
        builder.write(0x07, 4); // ECI mode
...
//# sourceMappingURL=DecodedBitStreamParser.spec.js.map