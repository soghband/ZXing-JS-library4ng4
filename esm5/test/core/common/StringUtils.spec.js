...
describe('StringUtils', function () {
    it('testShortShiftJIS_1', function () {
        // 金魚
        doTest(Uint8Array.from([/*(byte)*/ 0x8b, /*(byte)*/ 0xe0, /*(byte)*/ 0x8b, /*(byte)*/ 0x9b]), CharacterSetECI_1.default.SJIS.getName() /*"SJIS"*/);
    });
    it('testShortISO88591_1', function () {
        // båd
        doTest(Uint8Array.from([/*(byte)*/ 0x62, /*(byte)*/ 0xe5, /*(byte)*/ 0x64]), CharacterSetECI_1.default.ISO8859_1.getName() /*"ISO-8859-1"*/);
    });
    it('testMixedShiftJIS_1', function () {
        // Hello 金!
        doTest(Uint8Array.from([/*(byte)*/ 0x48, /*(byte)*/ 0x65, /*(byte)*/ 0x6c, /*(byte)*/ 0x6c, /*(byte)*/ 0x6f,
            /*(byte)*/ 0x20, /*(byte)*/ 0x8b, /*(byte)*/ 0xe0, /*(byte)*/ 0x21]), 'SJIS');
    });
    function doTest(bytes, charsetName) {
        // const charset: Charset = Charset.forName(charsetName);
        var guessedName = StringUtils_1.default.guessEncoding(bytes, null);
